...
export const OUTSOURCING_SETTING_FIELDS = [
  ...
  { key: "standardOutsourcingSqmUnitPrice", label: "標準請負単価 円/㎡" },
];
...
export const DEFAULT_OUTSOURCING_MODE = "labor";

export function normalizeStandardOutsourcingMode(mode) {
  return mode === "sqm" ? "sqm" : "labor";
}
...
export const DEFAULT_CLIENTS = [
  ...
  },
  {
    ...
    sheet: 800,
    ...DEFAULT_CLIENT_SETTINGS,
    standardOutsourcingMode: "labor",
    standardLaborUnitPrice: 23000,
  },
  {
    id: 3,
    name: "ハウスメンテナンス",
    ...
    sheet: 800,
    ...DEFAULT_CLIENT_SETTINGS,
    standardOutsourcingMode: "sqm",
    standardOutsourcingSqmUnitPrice: 850,
  },
];
...
export const DEFAULT_COMPANY = {
  ...
  standardLaborUnitPrice: DEFAULT_LABOR_UNIT_PRICE,
  monthlyTargetProfit: 500000,
  dailyTargetProfit: 20000,
  transportKmRate: 40,
  transportRoundTripDefault: true,
  vehicleName: "ハイエース",
  fuelEfficiencyKmPerL: 9,
  gasolinePricePerL: 200,
  googleMapsApiKey: "",
};
...
export function normalizeCompany(company) {
  return {
    ...
    monthlyTargetProfit: Number(company?.monthlyTargetProfit ?? 500000),
    dailyTargetProfit: Number(company?.dailyTargetProfit ?? 20000),
    transportKmRate: Number(company?.transportKmRate ?? 40),
    transportRoundTripDefault: company?.transportRoundTripDefault !== false,
    vehicleName: String(company?.vehicleName ?? DEFAULT_COMPANY.vehicleName),
    fuelEfficiencyKmPerL: Number(
      company?.fuelEfficiencyKmPerL ?? DEFAULT_COMPANY.fuelEfficiencyKmPerL
    ),
    gasolinePricePerL: Number(company?.gasolinePricePerL ?? DEFAULT_COMPANY.gasolinePricePerL),
    googleMapsApiKey: String(company?.googleMapsApiKey ?? ""),
  };
}
...
export function normalizeClient(client) {
  return {
    ...
    standardOutsourcingMode: normalizeStandardOutsourcingMode(
      client.standardOutsourcingMode ?? DEFAULT_OUTSOURCING_MODE
    ),
