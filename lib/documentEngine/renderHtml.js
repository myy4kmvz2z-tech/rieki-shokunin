...
function renderTable(viewModel) {
  const head = viewModel.tableHeaders.map((cell) => `<th>${escapeHtml(cell)}</th>`).join("");
  const body = viewModel.tableRows
    .map(
      (row) =>
...
/** 共通A4レイアウトからHTMLを生成 */
export function renderDocumentHtml(viewModel) {
  const company = viewModel.companyLines.map((line) => `<div>${escapeHtml(line)}</div>`).join("");
  const sites = viewModel.siteLines.map((line) => `<p>${escapeHtml(line)}</p>`).join("");
  const meta = viewModel.metaLines.map((line) => `<div>${escapeHtml(line)}</div>`).join("");
  const details = (viewModel.detailSections ?? [])
    .map(
      (section) =>
        `<section><strong>${escapeHtml(section.title)}</strong><div>${escapeHtml(section.body).replace(/\n/g, "<br />")}</div></section>`
    )
    .join("");
  const keyValues = (viewModel.keyValues ?? [])
    .map(
      (item) =>
        `<div class="kv"><span>${escapeHtml(item.label)}</span><span>${escapeHtml(item.value)}</span></div>`
    )
    .join("");
  const notes = (viewModel.notes ?? []).map((note) => `<p>${escapeHtml(note)}</p>`).join("");
...
<body>
  ...
  <div class="to">
    <strong>${escapeHtml(viewModel.clientLine)}</strong>
    ${sites}
  </div>
  <p>${escapeHtml(viewModel.intro)}</p>
  ${renderTable(viewModel)}
  <div class="summary">${renderSummary(viewModel)}</div>
